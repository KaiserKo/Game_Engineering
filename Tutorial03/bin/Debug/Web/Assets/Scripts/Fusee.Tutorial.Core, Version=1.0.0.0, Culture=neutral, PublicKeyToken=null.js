...
(function Tutorial$Members () {
  ...
  function Tutorial_Init () {
    ...
    (this.RenderCanvas$RC$value.ClearColor = $S01().Construct(0, 0, 0, 1));
  }; 
  ...
  function Tutorial_RenderAFrame () {
    ...
    this._baseRotation = +this._baseRotation + (+$T0B().get_Keyboard().get_ADAxis() * 0.05);
    this._upperArmRotation = +this._upperArmRotation + (+$T0B().get_Keyboard().get_WSAxis() * 0.05);
    if (+this._upperArmRotation < -0.6) {
      this._upperArmRotation = -0.6;
    } else {
      if (+this._upperArmRotation > 3.75) {
        this._upperArmRotation = 3.75;
      }
    }
    var key = $T0B().get_Keyboard().GetKey($T10().F);
    if (key) {
      if (+this._rotationSpeed < 0.5) {
        this._rotationSpeed = +this._rotationSpeed + 0.005;
      } else {
        this._rotationSpeed = 0.5;
      }
      this._forearmRotation = +this._forearmRotation + +this._rotationSpeed;
    } else {
    ...
    this._forearmMove = +this._forearmMove + (+$T0B().get_Keyboard().get_LeftRightAxis() * 0.015);
    if (+this._forearmMove < -0.45) {
      this._forearmMove = -0.45;
    } else {
      if (+this._forearmMove > 0.45) {
        this._forearmMove = 0.45;
    ...
    (this.RenderCanvas$RC$value).SetShaderParamfloat4x4(this._xformParam, this._xform.MemberwiseClone());
    (this.RenderCanvas$RC$value).Render(this._mesh);
    var upperArmModel = $thisType.ModelXForm($S00().Construct(0.4, -0.4, -0.2), $S00().Construct(0, 0, this._upperArmRotation), $S00().Construct(-0.4, 0, 0)).MemberwiseClone();
    this._xform = $S02().CallStatic($T06(), "op_Multiply", null, 
    ...
    (this.RenderCanvas$RC$value).SetShaderParamfloat4x4(this._xformParam, this._xform.MemberwiseClone());
    (this.RenderCanvas$RC$value).Render(this._mesh);
    var forearmModel = $thisType.ModelXForm($S00().Construct(this._forearmMove, 0, -0.11), $S00().Construct(0, 0, this._forearmRotation), $S00().Construct(0, 0, 0)).MemberwiseClone();
    this._xform = $S02().CallStatic($T06(), "op_Multiply", null, 
      ...
      $T06().CreateScale(0.35, 0.35, 0.01).MemberwiseClone()
    ).MemberwiseClone();
    (this.RenderCanvas$RC$value).SetShaderParamfloat4x4(this._xformParam, this._xform.MemberwiseClone());
    (this.RenderCanvas$RC$value).Render(this._mesh);
    this.Present();
  ...
  function Tutorial_stopRotation () {
    if (+this._rotationSpeed > 0) {
      this._rotationSpeed = +this._rotationSpeed - 0.005;
    } else {
      if (+this._rotationSpeed <= 0) {
  ...
  JSIL.MakeType({
    ...
    $.Method({Static:false, Public:false}, "stopRotation", 
      JSIL.MethodSignature.Void, 
      Tutorial_stopRotation
    );
    ...
    $.Field({Static:false, Public:false}, "_forearmMove", $.Single);

    $.Field({Static:false, Public:false}, "_rotationSpeed", $.Single);
